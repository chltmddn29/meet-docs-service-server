...
- speaker_points(발언자별 정리): "누가 무엇을 말했는지/맡았는지"를 "이름: 내용" 형식으로 정리하세요.
  * 회의록 텍스트에 "화자1:", "화자2:" 같은 라벨이 이미 있다면, 이는 음성분석으로
    실제 목소리를 구분한 결과이니 1차 근거로 삼으세요(텍스트만으로 추측한 게 아님).
  * 참석자 명단과 문맥(화자 라벨 근처의 이름 언급, "제가/내가" 등)을 보조 근거로 써서
    가능하면 "화자1" 대신 실제 이름으로 바꿔 쓰세요. 이름을 확정할 근거가 없으면
    화자 라벨("화자1" 등)을 그대로 이름 자리에 쓰세요.
  * 화자 라벨이 없는 텍스트라면 기존처럼 명단·문맥으로 합리적으로 추정 가능한
    경우에만 이름을 붙이고, 불확실하면 그 항목은 넣지 마세요(억지로 배정 금지).
